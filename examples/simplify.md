---
name: 项目精简与维护成本门禁
description: 适用于审视项目是否存在过度设计、过度校验和不必要兼容逻辑，并通过门禁循环持续降低维护成本的任务。
---

```mermaid
flowchart TD
  start((开始)) --> simplify[执行项目精简]
  simplify -->|已精简| finish((结束))
  simplify -->|无法精简| finish((结束))
  simplify -->|继续精简| simplify
```

## 执行项目精简

```新建Agent
审视该项目是否存在过度设计、过度校验的问题，能不能考虑不是修补打补丁的方案，而是考虑问题的共性和降低项目维护成本，不要做任何历史兼容工作，任何兼容性的工作都是可以删除的，做一次精简。如果已经无法精简了，可以汇报原因。

请按以下原则完成本节点：

- 先理解项目中待处理问题的共性、重复模式和长期维护成本，再决定修改方案。
- 优先采用能够减少抽象、分支、重复校验和特殊路径的整体方案，不要只做局部补丁。
- 不要保留任何仅为历史版本、旧调用方或旧数据格式服务的兼容逻辑；兼容性工作可以删除。
- 实际完成可以验证的精简，并说明删除或合并了什么，以及为什么不会改变当前需求语义。
- 本节点同时负责门禁判断：如果本轮精简后仍有可以整体处理的共性复杂度，提交“继续精简”；如果已经达到合理的精简边界，提交“已精简”；如果本轮没有找到可行的精简方案，提交“无法精简”。
- 提交“无法精简”时，必须在 content 中汇报检查范围、保留项和无法继续精简的具体原因。

{outcome}
```

### 已精简

已完成合理范围内的整体精简，并提供了修改范围、共性问题和验证证据，当前门禁允许流程结束。

### 无法精简

已审视当前项目，没有找到能够在不改变当前需求语义的前提下继续降低复杂度的方案，并在结果中说明了检查范围和原因。

### 继续精简

本轮精简已经完成，但仍发现可以整体删除、合并或简化的共性复杂度；结果中已附上本轮修改和验证证据，流程将进入下一轮。