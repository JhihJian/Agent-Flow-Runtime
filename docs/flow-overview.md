...
目录组织约定见[Flow 规范](flow-spec.md)中的“Flow 包目录”章节。运行时只执行以`FLOW.md`为入口的目录包。
...
`FLOW.md`父目录名是 Flow 标识，供目录发现和运行记录关联。入口文件只描述通用路径，不包含某次任务的输入、执行结果或 Agent 会话。
...
`执行Flow`节点复用另一条成熟路径：父图保持自身结构，子 Flow 走完自己的门禁与返工后，父节点拿到子 Run 终态继续路由。子 Run 有独立的运行记录和 Agent 绑定，只随父 Run 恢复。写法与语义见[Flow 规范](flow-spec.md)中的引用执行章节。
