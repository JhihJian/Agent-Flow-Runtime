...
负责保存和查询 Run、NodeRun 及并行轮次关联。它不决定流程路径。
...
如果节点执行、结果校验或路径查找失败，Run 进入失败状态，当前 NodeRun 也应保留失败信息。当前实现支持恢复执行中的 Run；Agent 节点会创建新的访问继续执行，未完成的命令节点不会被自动重放，而是直接将 Run 标记为失败。
...
当前实现尚未保存 Flow 内容版本指纹。要严格复现历史 Run，后续应在 Run 创建时保存 Flow 定义的版本或内容摘要。
...
运行记录查询目前通过`RunStore`接口提供，扩展层主要在流程结束或失败时通知用户。持续进度展示和命令查询入口仍属于后续能力。命令节点中断后的安全失败和 Agent 节点恢复已经由 Runtime 提供，但更细的节点失败原因和状态分类仍可继续完善。