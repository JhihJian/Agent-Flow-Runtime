...
它表示“这次经过了什么”。同一节点被返工多次时，每次进入都会产生新的 NodeRunRecord。`执行Flow`节点还会记录启动的子 Run 标识；子 Flow 的全部事实写入子 Run，父 Run 中没有子节点记录。
...
负责读取和校验 Flow 包，生成可信的`FlowDefinition`，并构建引用闭包：按兄弟包解析`执行Flow`引用，递归加载子包及其资源，在加载期拒绝缺失与环引用。
...
- 校验节点结果并决定下一去向。
- 创建并行轮次，等待分支并执行汇合。
- 执行`执行Flow`节点时，从闭包注册表组装子协调器，复用同一 RunStore、AgentRunModel、命令执行器和观测发布器。
- 在完成或失败时结束 Run。
...
Run 创建时必须保存 Flow 内容版本或稳定指纹，否则恢复和历史查询不得声称能够复现当时的 Flow。引用执行的 Run 保存组合指纹，递归包含全部子 Flow 指纹，子 Flow 文件漂移与父文件漂移同样会被发现。Task 仍属于 Run 的输入快照或稳定引用，不得写入 Flow 定义。
...
当前实现包含 Flow 解析、普通节点流转、Agent 新建和复用、命令执行、单层命令并行、显式汇合、Flow 引用执行（无结果边直连与「已完成 / 已失败」双结果边）、Pi 会话关联、本地运行记录，以及 Pi CLI 恢复执行中的 Run。子 Run 只随父 Run 恢复，父子聚合视图属于观测层后续工作。
