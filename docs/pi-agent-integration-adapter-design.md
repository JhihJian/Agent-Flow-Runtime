...
MVP 支持 SDK 内嵌会话的新建与恢复、Pi CLI 通过扩展注入`/flow-new-session`完成会话替换、单会话串行节点执行、结构化结果提交、节点交互查询和流程结束释放。该命令使用独立名称，避免与 Pi 内置交互命令`/new`冲突。每个 CLI `新建Agent`都会替换为清空上下文的新会话，`复用Agent`继续当前会话。CLI 路径会在新会话建立后恢复启动 Flow 时的当前模型，避免落回 Pi 全局默认模型；工具白名单、会话目录等其他运行环境配置由外部宿主或 Pi CLI 提供。
