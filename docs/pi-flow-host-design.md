...
当前实现通过统一 Runtime 入口提供 `/flow list`、`/flow show <runId>`、`/flow web [port]`、JSON/RPC `flow_event` 消息和 Agent 的只读 `inspect_flow_run` 工具。`/flow web`启动带令牌的本地只读 Web 观察站，`/flow web stop`关闭服务。复杂运行记录、节点证据和命令输出由 [Web 展示设计](flow-observability-web-design.md)定义。重连时先读取 Inspector 快照，再订阅后续事件；持续事件为尽力而为通知，不补发断线期间的全部事件。
