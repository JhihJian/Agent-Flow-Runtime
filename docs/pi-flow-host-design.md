...
当前实现通过统一 Runtime 入口提供 `/flow list`、`/flow show <runId>`、TUI 状态栏/小组件、JSON/RPC `flow_event` 消息和 Agent 的只读 `inspect_flow_run` 工具。`/flow list` 展示近期运行并在可交互宿主中提供选择，选择后复用同一历史投影。重连时先读取 Inspector 快照，再订阅后续事件；持续事件为尽力而为通知，不补发断线期间的全部事件。
