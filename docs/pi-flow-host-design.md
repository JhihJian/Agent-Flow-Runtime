...
当前实现通过统一 Runtime 入口提供 `/flow list`、`/flow show <runId>`、`/flow web [port]`、`/flow web lan [port]`、JSON/RPC `flow_event` 消息和 Agent 的只读 `inspect_flow_run` 工具。`/flow web`启动带令牌的 loopback Web 观察站，`/flow web lan`作为显式确认监听`0.0.0.0`并输出局域网 URL，`/flow web stop`关闭服务。LAN 模式只适用于可信网络；公网访问需要 TLS 和额外网络认证。复杂运行记录、节点证据和命令输出由 [Web 展示设计](flow-observability-web-design.md)定义。重连时先读取 Inspector 快照，再订阅后续事件；持续事件为尽力而为通知，不补发断线期间的全部事件。
