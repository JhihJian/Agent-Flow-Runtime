...
| 子 Flow 的结果 | 固定两条结果边「已完成 / 已失败」，运行时按子 Run 终态路由 |
| 子 Flow 失败 | 走「已失败」结果边，失败详情在结果内容里 |
...
- 双结果路由的是机器事实：子 Run 终态（completed / failed / interrupted）由运行时判定，「已完成 / 已失败」按此选择，全程不经 Agent 判断；依据`result`内容做更深层业务判断（例如返工）仍可由后续 Agent 节点承担。
- 与命令节点保持单结果不同：命令退出码是单次进程的细粒度信号，子 Flow 终态是整条子路径的门禁结论；子 Flow 执行成本高，失败去向值得在图中显式声明。
...
子 Flow 已走完自身路径，结果内容携带其最终结论。

### 已失败

子 Flow 未能走完自身路径，结果内容携带失败详情。
````
...
- `status`：子 Run 状态为 completed 时取 success 并走「已完成」，failed 或 interrupted 时取 failure 并走「已失败」。
...
3. 子 Run 启动并运行到终态；父节点映射终态为结果内容，并按「已完成 / 已失败」结果边推进。
...
2. 父 NodeRun 中断、子 Run 已终态：读取子 Run 终态，映射结果内容并按「已完成 / 已失败」补全父节点结果，继续路由。子 Run 完成与父节点完成是两次提交，崩溃落在这两次之间时由此路径闭合。
...
- 「执行Flow」节点恰有「已完成」和「已失败」两条结果边，缺一即拒绝。
...
| parser.ts | 解析`执行Flow`标记；命令节点保持「已执行」单结果，「执行Flow」节点校验「已完成 / 已失败」双结果 |
...
- 子 Run 完成后父节点走「已完成」边，failed / interrupted 走「已失败」边，结果内容形状正确。
- 「执行Flow」节点缺少「已完成」或「已失败」结果边被解释器拒绝。
