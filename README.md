...
In Pi, use `/flow run <文件> <任务>`, `/flow list` for recent Run IDs, or `/flow show <runId>` for plain-text history. Use `/flow web` to start the loopback Web observation host on port `3818`, or `/flow web <port>` to choose a port. Use `/flow web lan [port]` to explicitly open the host on `0.0.0.0` for a trusted LAN; Pi prints the tokenized URL, and `/flow web stop` closes the host. LAN mode uses HTTP and therefore requires a trusted network. Public access requires TLS and network authentication deployment. In RPC mode, send a normal `prompt` request after starting Pi with `--flow`; the extension intercepts it and drives the full Flow. JSON and RPC streams receive `flow_event` custom messages whose `details` contain the structured event, rather than using Agent prose to infer state.
