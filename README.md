...
In Pi, use `/flow run <文件> <任务>`, `/flow list` for recent Run IDs, or `/flow show <runId>` for plain-text history. The package no longer renders an interactive TUI preview; detailed interactive inspection belongs to the planned Web observation host. In RPC mode, send a normal `prompt` request after starting Pi with `--flow`; the extension intercepts it and drives the full Flow. JSON and RPC streams receive `flow_event` custom messages whose `details` contain the structured event, rather than using Agent prose to infer state.
