...
```bash
pi install npm:@jhihjian/agent-flow-runtime@0.1.3
```
...
- `src/extension.ts` registers `--flow`, `/flow run`, and `submit_flow_outcome`. CLI outcomes are only confirmed at Pi's `agent_settled` boundary, after Pi's native compaction, retries, and queued work have completed.
