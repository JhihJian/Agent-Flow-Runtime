...
In Pi, use `/flow run <文件> <任务>`, `/flow list` for recent Run IDs, or `/flow show <runId>` for plain-text history. Use `/flow web` to start the local Web observation host on port `3818`, or `/flow web <port>` to choose a port. Pi prints a tokenized loopback URL; open it in a browser to inspect Run facts and NodeRun evidence. `/flow web stop` closes the host. The Web host listens only on `127.0.0.1`; LAN or public access requires a separate TLS and network authentication deployment. In RPC mode, send a normal `prompt` request after starting Pi with `--flow`; the extension intercepts it and drives the full Flow. JSON and RPC streams receive `flow_event` custom messages whose `details` contain the structured event, rather than using Agent prose to infer state.
